package com.hih.pcbcutter.util;
...
/**
 * Created by likeye on 2020/8/20 15:39.
 **/
...
//            sheet.addCell(label);
     ...
     * @param <T>
     */
    public static <T> void writeObjListToExcel(int flag,List<T> objList, String fileName, Context context) {
        if (objList != null && objList.size() > 0) {
            ...
            try {
                ...
                for (int j = 0; j < objList.size(); j++) {
                    ...
                    if(flag==0) {
                        ProductRecord demoBean = (ProductRecord) objList.get(j);
                        list.add(demoBean.getId() + "");
                        list.add(String.valueOf(demoBean.getDate()));
                        list.add(String.valueOf(demoBean.getStart()));
                        list.add(String.valueOf(demoBean.getEnd()));
                        list.add(String.valueOf(demoBean.getMessage()));
                    }else{
                        ErrorMsg errorMsg=(ErrorMsg)objList.get(j);
                        list.add(errorMsg.getNum());
                        list.add(errorMsg.getTime());
                        list.add(errorMsg.getMsg());
                    }
