...
import com.hih.pcbcutter.pic.bean.BrokenLine;
import com.hih.pcbcutter.pic.bean.Circle;
import com.hih.pcbcutter.pic.bean.Line;
import com.hih.pcbcutter.pic.bean.Operation;
import com.hih.pcbcutter.pic.bean.Oval;
import com.hih.pcbcutter.pic.bean.Polygon;
import com.hih.pcbcutter.pic.bean.Rect;
...
public class PicView extends View {
    ...
    //椭圆集合
    private final List<Oval> ovalList = new ArrayList<>();
    //多边形集合
    private final List<Polygon> polygonList = new ArrayList<>();
    //折线集合
    private final List<BrokenLine> brokenLineList = new ArrayList<>();
    //操作集合
    private final List<Operation> operationList = new ArrayList<>();
    //矩形删除集合
    private final List<Rect> rectListDel = new ArrayList<>();
    //椭圆删除集合
    private final List<Oval> ovalListDel = new ArrayList<>();
    //移动
    ...
    //图中坐标
    private PointF downPointB;
    private PointF movePointB;
    private PointF upPointB;
    //圆半径
    ...
    private boolean mIsMove;//是否双指拖动图片中ing
    ...
    private Bitmap savebtm;
    public void setChooseListenr(ChooseListener listener) {
        this.listener = listener;
    }
    public PicView(Context context, Bitmap bitmap) {
        super(context);
        init(bitmap);
    }
    ...
    private void init(Bitmap bitmap) {
        mSrcBitmap = bitmap.copy(Bitmap.Config.ARGB_8888, true);
...
//        mSrcBitmap = BitmapFactory.decodeResource(getResources(), R.drawable.demobg,options);
        ...
        paintDel = new Paint();
        paintDel.setColor(Color.RED);
        paintDel.setStyle(Paint.Style.STROKE);
        paintDel.setStrokeWidth(5);
        paintDel.setAntiAlias(true);
    ...
    private void lineChange() {
        if (flag == 0 && chooseLine > 0) {
            ...
            if (move) {
                ...
                mCurrentCanvas.drawLine(pp1.x, pp1.y, pp2.x, pp2.y, lineTemp.getPaint());
            ...
            if (clip2) {
                ...
                mCurrentCanvas.drawLine(pp1.x, pp1.y, movePointB.x, movePointB.y, lineTemp.getPaint());
            ...
            if (clip1) {
                ...
                mCurrentCanvas.drawLine(movePointB.x, movePointB.y, pp2.x, pp2.y, lineTemp.getPaint());
            }
            if (!touchMove) {
    ...
    private void circleChange() {
        if (flag == 0 && chooseCircle > 0) {
            ...
            if (move) {
                ...
                mCurrentCanvas.drawCircle(pp1.x, pp1.y, circleTemp.getR(), circleTemp.getPaint());
                mCurrentCanvas.drawCircle(pp1.x, pp1.y, 10, paint);
            }
            if (sCirle) {
                reMoveR(circleList, circleTemp);
                circleR = (float) (lineSpace(movePointB.x, movePointB.y, circleTemp.getPoint().x, circleTemp.getPoint().y));
                mCurrentCanvas.drawCircle(circleTemp.getPoint().x, circleTemp.getPoint().y, circleR, circleTemp.getPaint());
    ...
    private void rectChange() {
        if (flag == 0 && chooseRect > 0) {
            ...
            if (move) {
                ...
                mCurrentCanvas.drawRect(pp1.x, pp1.y, pp2.x, pp2.y, rectTemp.getPaint());
                mCurrentCanvas.drawCircle(pp3.x, pp3.y, 10, paint);
            }
            /*if (clip1) {
                if (ff) {
            ...
            if (isClip1) {
                ...
                mCurrentCanvas.drawRect(rectTemp.getP1().x, pxd.y, rectTemp.getP2().x, rectTemp.getP2().y, rectTemp.getPaint());
            }
            if (isClip2) {
                reMoveR(rectList, rectTemp);
                mCurrentCanvas.rotate((float) rectTemp.getAngle(), rectTemp.getPmid().x, rectTemp.getPmid().y);
                mCurrentCanvas.drawRect(rectTemp.getP1().x, rectTemp.getP1().y, rectTemp.getP2().x, pxd.y, rectTemp.getPaint());
            }
            if (isClip3) {
                reMoveR(rectList, rectTemp);
                mCurrentCanvas.rotate((float) rectTemp.getAngle(), rectTemp.getPmid().x, rectTemp.getPmid().y);
                mCurrentCanvas.drawRect(pxd.x, rectTemp.getP1().y, rectTemp.getP2().x, rectTemp.getP2().y, rectTemp.getPaint());
            }
            if (isClip4) {
                reMoveR(rectList, rectTemp);
                mCurrentCanvas.rotate((float) rectTemp.getAngle(), rectTemp.getPmid().x, rectTemp.getPmid().y);
                mCurrentCanvas.drawRect(rectTemp.getP1().x, rectTemp.getP1().y, pxd.x, rectTemp.getP2().y, rectTemp.getPaint());
            }
            if (rotate) {
                reMoveR(rectList, rectTemp);
                ...
                mCurrentCanvas.drawRect(rectTemp.getP1().x, rectTemp.getP1().y, rectTemp.getP2().x, rectTemp.getP2().y, rectTemp.getPaint());
                mCurrentCanvas.rotate((float) -angle, (float) (left + (right - left) / 2), (float) (top + (bottom - top) / 2));
            }
           /* if(clip3){
                pp5 = new PointF();
    ...
    private void ovalChange() {
        if (flag == 0 && chooseOval > 0) {
            ...
            if (move) {
                ...
                mCurrentCanvas.drawOval(rectF, ovalTemp.getPaint());
            }
            if (isClip1) {
                reMoveR(ovalList, ovalTemp);
            ...
            if (rotate) {
                ...
                //以椭圆中心点旋转图形
                mCurrentCanvas.drawOval(ovalTemp.getRectF(), ovalTemp.getPaint());
                //mCurrentCanvas.rotate((float) angle, ovalTemp.getPmid().x, ovalTemp.getPmid().y);
            }
            if (!touchMove) {
            ...
            for (int i = 0; i < brokenLineTemp.getBList().size(); i++) {
                if (i == choosePoint) {
                    ...
                    for (int j = 1; j < brokenLineTemp.getBList().size(); j++) {
                        mCurrentCanvas.drawLine(brokenLineTemp.getBList().get(j - 1).x, brokenLineTemp.getBList().get(j - 1).y, brokenLineTemp.getBList().get(j).x, brokenLineTemp.getBList().get(j).y, brokenLineTemp.getPaint());
                    }
                    break;
                }
            }
        ...
        if (flag == 0 && choosePolygon > 0) {
            ...
            for (int i = 0; i < polygonTemp.getPointFList().size(); i++) {
                if (i == choosePoint) {
                    ...
                    for (int j = 1; j < polygonTemp.getPointFList().size(); j++) {
                        mCurrentCanvas.drawLine(polygonTemp.getPointFList().get(j - 1).x, polygonTemp.getPointFList().get(j - 1).y,
                                polygonTemp.getPointFList().get(j).x, polygonTemp.getPointFList().get(j).y, polygonTemp.getPaint());
                    }
                    if (polygonTemp.getPointFList().size() > 2) {
                        mCurrentCanvas.drawLine(polygonTemp.getPointFList().get(0).x, polygonTemp.getPointFList().get(0).y,
                                polygonTemp.getPointFList().get(polygonTemp.getPointFList().size() - 1).x,
                                polygonTemp.getPointFList().get(polygonTemp.getPointFList().size() - 1).y, polygonTemp.getPaint());
                    }
                    break;
                }
            }
...
//            rect.setW(Math.abs(downPointB.y - upPointB.y));
            rect.setAngle(rectAngle);
            if (delChoose) {
                rect.setPaint(paintDel);
                rectListDel.add(rect);
            } else {
                rectList.add(rect);
     ...
     * 画箭头
     */
    private void drawAL(PointF pf1, PointF pf2) {
        float sx = pf1.x;
        float sy = pf1.y;
        float ex = pf2.x;
        float ey = pf2.y;
    ...
    //长按选中
    private void longChoose(PointF p) {
        chooseLine = 0;
        chooseCircle = 0;
        chooseRect = 0;
        chooseOval = 0;
     ...
     * @return
     */
    private Bitmap getTransparentBitmap(Bitmap sourceImg) {
    ...
    /**
     * 椭圆公式（x-h)*(x-h)/(a*a)+(y-k)*(y-k)/(b*b)=1
     *
     * @param p1
     * @param oval
     */
    //判断点是否在椭圆内
    private boolean pointInOval(PointF p1, Oval oval) {
        boolean flag = false;
        double temp = (p1.x - oval.getPmid().x) * (p1.x - oval.getPmid().x) / ((oval.getL() / 2) * (oval.getL() / 2)) +
                (p1.y - oval.getPmid().y) * (p1.y - oval.getPmid().y) / ((oval.getW() / 2) * (oval.getW() / 2));
        if (temp > 1) flag = false;
        else if (temp < 1 && temp > 0) flag = true;
        //  Log.d("SS","在="+flag);
        return flag;
    }
    // 还原堆栈中的记录的操作
    private void draw1(Canvas canvas) {
        //canvas=new Canvas(mSrcBitmap);
        Random r = new Random(1);
        boolean flagP;
        ...
        for (int i = 0; i < ovalList.size(); i++) {
            for (int j = 0; j < 100000; j++) {
                ...
                pointF.x = x;
                pointF.y = y;
                if (pointInOval(pointF, ovalList.get(i))) {
                    canvas.drawPoint(x, y, paint);
                }
            }
        }
       ...
       // savebtm=Bitmap.createBitmap(mSrcBitmap);
    }
    /**
     * 将触摸的屏幕坐标转换成实际图片中的坐标
     */
    ...
    //点到矩形的距离
    private double pointToRect(Rect rect, PointF p) {
        double dmin;
    ...
    //移除选中操作图形重画
    private void reMoveR(List<?> list, Object ob) {
        if (ff) {
            ff = false;
            list.remove(ob);
            initCanvas();//添上这句防止重复绘制
    ...
    //三点的夹角
    private static double calcAngle(PointF cen, PointF first, PointF second) {
...
//          System.out.println("不变");
    ...
    //线段与x=a或y=a的交点
    private PointF getLineCross(PointF p1, PointF p2, float a, boolean yp) {
        float A = p2.y - p1.y;
        float B = p1.x - p2.x;
    ...
    private static boolean between(float a, float b, float target) {
    ...
    public void saveBitmap(String picName) {
        File folder = new File("/sdcard/picTest/");
        if (!folder.exists()) {
            folder.mkdir();
        }
        SaveFileThread saveFileThread = new SaveFileThread("/sdcard/picTest/" + picName, savebtm);
        saveFileThread.run();
    }
    private static class SaveFileThread implements Runnable{
        private final String mPath;
        private final Bitmap mBitmap;
        SaveFileThread(String path, Bitmap bitmap){
            this.mPath=path;
            this.mBitmap=bitmap;
        }
        @Override
